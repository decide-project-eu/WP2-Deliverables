...
get.mu0 <- function(Data, stratify.by=NA, time.var, expected.start.time, relevant.names, simple.linear = FALSE, plot.it=FALSE){
...
get.C0 <- function(Data, stratify.by, time.var, expected.start.time, relevant.names){
  ...
  for(stratification.group in sort(unique(D.full[,stratify.by]))){
   ...
   #  - omit outliers, as these are probably caused by switching from one calf to another
    outlier.i <- which(Diff > mean(Diff)+2*sd(Diff) | Diff < mean(Diff)-2*sd(Diff) )
    Data.A <- Data.A[-outlier.i,]
    Data.A <- na.omit(Data.A)
    # if it is possible, we should only consider the days around the expected start.time
    time.unit <- median(diff(Data[,time.var]))
    Data.AA <- subset(Data.A, Data.A[,time.var] %in% (expected.start.time-2*time.unit):(expected.start.time+2*time.unit) )
    if(nrow(Data.AA) > 5){
      C0 <- cov(Data.AA[,C0.names])
    }else{
  ...
  for(name in relevant.names){ 
    ...
    if(plot.it == TRUE){
      for(i in 1:length(spline.list.name)){
        ...
        if(i == 1){
          plot(Spline$y~Spline$x, ylim=range(for.limits), xlab=time.var, ylab=name, type='l', lty=i, lwd=2)
        }else{
...
runDLM <- function(Data, 
  ...
  return(list(
    ...
    vse=VSE.list,
    Gt.list=Gt.list,
    d2=d2.list,  #  <-- THIS IS NEW!
    d2_scaled=d2_scaled.list,  #  <-- THIS IS NEW!
    p_value=p_value.list,  #  <-- THIS IS NEW!
    ucl=ucl.list  #  <-- THIS IS NEW!
  ))
}
...
optimize.delta <- function(deltas=seq(from=0.5, to=1, by=0.01), Learningset, identifyer, mu0.list, C0.list, V.list, relevant.names, Spline.list=NA, time.var){
  ...
  for(stratification.group in unique(Learningset.full[,stratify.by])){
    ...
    best.delta.list[[length(best.delta.list)]] <- best.delta 
...
extract.res <- function(res){
  ...
  # Get the filetered means 
  df_mt <- data.frame(t(sapply(res$mt,c)))
  colnames(df_mt) <- paste('mt_', rownames(res$mt[[1]]),sep='')
  df <- cbind(df, df_mt)
  ...
  # Get the diagonals of Qt, i.e. the forecast variance for each variable
  df_Qt <- as.data.frame(
    do.call(rbind, lapply(res$Qt, diag)),
    row.names = paste0('Qt_', names(res$Qt))       # optional: keeps list names as row names
  )
  colnames(df_Qt) <- paste0('Qt_', colnames(df_Qt))
  df <- cbind(df, df_Qt)
  
  # Get the output from the Mahalanobis distance method
  df$d2 <- unlist(res$d2)
  df$d2_scaled <- unlist(res$d2_scaled)
  df$p_value <- unlist(res$p_value)
  df$ucl <- unlist(res$ucl)
