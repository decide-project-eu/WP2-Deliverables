...
# Use the *get.mu0* (from the sourced functions file) function to make the initial parameter vector, mu0, and  *get.C0* to make the initial variance matrix, C0, for milk yield, and call the outputs "outmu" and "outc". Use the following settings when applying for the function:
...
Gt <- get.Gt(Data.A=healthy.set,
             ...
             relevant.names.A=c('yield'))

Ft <- get.Ft(relevant.names.A = c('yield'))
...
for(ID in IDs.healthy){
  ...
  extracted <- extract.res(res = res, relevant.names=c('yield'))
...
for(ID in IDs.sick){
  ...
  ut <- extracted$ut_yield
  ut.all.sick <- c(ut.all.sick, ut)
}

# Now that we have applied the optimized DLM to the data from cows with mastitis and extracted the standardized forecast errors, we wish to assess the performance we can achieve when using the standardized forecast errors of the DLM as part of a mastitis alarm system. 

# Apply the *alarmsMontgomery* function from the sourced "DLM functions_DECIDE" script, and call the result "alarms"; use the following settings:

alarms <-  alarmsMontgomery(k = ut.all.sick, 
                            cl = mean(ut.all.sick), 
                            SD = sd(ut.all.sick), 
                            Ylim = NA, 
                            Main = 'yield', 
...
# Use the *colnames* function to make a vector called "Rules", containing the column names of the "alarms" data set. 

Rules <- colnames(alarms)

# Apply the *getPerformance* function from the sourced script, and call the result "performance"; use the following settings:

# observations = sick.set[,'mastitis']
# alarms = alarms[,'AnyRule']
...
## Univariate DLM on multiple data series ##

# Now we wish to estimate the parameters (mu0, C0, V, and delta) for the DLM to describe the healthy state of each of the numerical variables, which we standardized earlier.

# It is interesting to be able to tell how long it takes to run the the entire chunk of code, used for this task. For this purpose, use the *Sys.time* function to create an object called "start.time". 

start.time <- Sys.time()

# Then, make an empty data frame called "performances.univariate"; we will use this to store the performances for all the numerical variables for easy comparison.

performances.univariate <- data.frame()

# Make an empty vector called "all.deltas"; we will use this to collect the optimized delta values for the seven numerical variables.

all.deltas <- c()

#Run the for-loop to iterate over all the relevant numerical variables in the vector "names", repeating the process done earlier just for milk yield.
...
for(name in names){
  ...
  # Define mu0 and C0
  out <- get.mu0_C0(Data = healthy_start.set, 
                    time.var = 'dim', 
                    relevant.names = c(name))
  mu0 <- out$mu0
  C0 <- out$C0
  
  # Define Gt and Ft
  Gt <- get.Gt(relevant.names = c(name))
  Ft <- get.Ft(relevant.names = c(name))
  
  # Define V
  V <- get.V(Data = healthy.set, ID.var = 'Cow.Parity', relevant.names = name)
  
  # Optimize the value of delta
  delta <- optimize.delta(Data = healthy.set, 
                          ID.var = 'Cow.Parity', 
                          deltas = seq(from=0.8, to=1, by=0.01), 
                          mu0 = mu0, C0 = C0, V = V,
                          relevant.names = c(name))
  
  all.deltas <- c(all.deltas, delta)
  
  # Apply the optimized DLM to the individual cow-parities in the healthy.set estimate the Mean and SD to be used for the control chart
  ut.all <- c()
  for(ID in IDs.healthy){
    ID.set <- subset(healthy.set, healthy.set$Cow.Parity == ID)
    res <- runDLM(Data = ID.set, 
                  mu0 = mu0, 
                  C0 = C0, 
                  V = V, 
                  delta = delta,
                  relevant.names = c(name))
    extracted <- extract.res(res = res, relevant.names = c(name))
    ut.all <- c(ut.all, extracted[,paste('ut_', name, sep='')])
  }
  
  # Calculate the Mean and SD values for the standardized forecast errors of this variable
  Mean <- mean(ut.all)
  SD <- sd(ut.all)
  
  # Get the alarms based on the four rules of Montomery and extract the performance of *AnyRule*
  ut.all <- c()
  for(ID in IDs.sick){
    ID.set <- subset(sick.set, sick.set$Cow.Parity == ID)
    res <- runDLM(Data = ID.set, 
  ...
  # Get the alarms based on the four rules of Montomery
  alarms <-  alarmsMontgomery(k = ut.all, 
                              cl = Mean, 
                              SD = SD, 
                              Ylim = NA, 
                              Main = name, 
  ...
  # Get the performance based on "AnyRule"
  performance <- getPerformance(
    observations = sick.set[,'mastitis'],
    alarms = alarms[,'AnyRule'])
  
  # Add a column with the name to the performance
  performance <- cbind('Name'=name, performance)
  
  # Add the performance to the performances data frame
  performances.univariate <- rbind(performances.univariate, performance)
}
...
print(difftime(time1 = end.time, time2 = start.time, units = 'min'))

print(performances.univariate)

########################################################################

## Multivariate DLM ##

# Now we wish to make a single multivariate DLM, which simultaneously describe all the numerical variables, while taking their mutual co-variances into account. 

# First, we create the initial parameters, mu0 and C0, for the multivariate DLM. 

# Apply the *get.mu0_C0*, and call the resulting list "out"; use the following settings:

# Data = healthy_start.set
# time.var = 'dim'
# relevant.names = names

out <- get.mu0_C0(Data = healthy_start.set, time.var = 'dim', relevant.names = names)
mu0 <- out$mu0
C0 <- out$C0

# Use the *get.Gt* and  the *get.Ft* functions to create the *Gt* and *Ft* matrices for the multivariate DLM; in both functions, use the following settings:

Gt <- get.Gt(relevant.names = names)
Ft <- get.Ft(relevant.names = names)
Gt
Ft

# Estimate the observational variance-covariance matrix (*V*): use the *get.V* function with the following settings:

# Data = healthy.set
# ID.var = 'Cow.Parity'
# relevant.names = names

V <- get.V(Data = healthy.set, ID.var = 'Cow.Parity', relevant.names = names)
round(V,3)

# Now we wish to find the optimal *delta* value for the multivariate DLM.Use the *optimize.delta* function from the *DLM functions.R* script to find the optimal delta value for modelling yield with the DLM. Apply the function with the following settings:
...
# Now we wish to apply multivariate DLM to the individual cow-parities in the *sick.set*. 
# We will not test the performances of each variable on their own again, because in this specific case, the performances would be comparable to what we saw before. Instead, we will save the data that we can extract from the output of the DLM. 
...
sick.set_after.DLM <- cbind(meta.data.sick, extracted.all.sick)

saveRDS(object = sick.set_after.DLM, file = "sick.set_after.DLM.RDS")