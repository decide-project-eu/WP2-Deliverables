...
# This first example will go through the multivariate DLM.
...
### Make a vector called "relevant.names", which includes the variables to be monitored simultaneously. Use the the following column names:

relevant.names <- c("log.d9.sal", "log.max.daily.range.sal","log0.mortality.rel.20")

## We will only use mortality and salinity related variables as it was shown in previous work that these were the only relevant variables to monitor salmon mortality in Scotland. 
## The original environmental data was reported on a daily basis while the mortality data is monthly data. To have all the data within the same time span, we aggregated the environmental data per month. *log.d9.sal* corresponds to the log transformation of the 9th decile of salinity, *log.max.daily.range.sal* corresponds to the log transformation of the maximum daily variation of salinity out of a month, and *log0.mortality.rel.20* corresponds to the log transformation of the relative mortality.
...
# When variables differ too much in range and magnitude, larger numerical changes can drive the model, so it is customary to standardize the data in multivariate DLMs. So we will standardize now. 
...
### Multivariate DLM on salmon mortality ###
...
# time.var = c("n.months", "n.months", "months.since.start"). This is a vector with one time variable for each relevant.name considered. 'n.months' is the time variable for the salinity related variables and corresponds to the number of months since the beginning of the dataset. Salinity did not show any seasonality in our study (so using the months of the year did not make sense), and it does not change according to the production cycles (so using the 'months.since.start' did not make sense either). 'months.since.start' is used for monitoring the mortality of salmon for each production cycle, and signifies the months since salmon are stocked into the sea until they are harvested.
...
# Use the get.Gt and get.Ft functions to create the Gt and Ft matrices, respectively.
...
Gt <- get.Gt(Data.A=Learning.set,
             i.A=NA,
             time.var.A=c("n.months","n.months","months.since.start"), 
             stratify.by.A=NA, 
             Spline.list.A=Spline.list, 
             relevant.names.A=c("log.d9.sal","log.max.daily.range.sal","log0.mortality.rel.20"))

Ft <- get.Ft(relevant.names.A = c("log.d9.sal","log.max.daily.range.sal","log0.mortality.rel.20"))

# Use function *get.V* to find the observational variance, V. Use the following settings:
...
## Apply the optimized DLM to the relevant.names of each production cycle (nseq) in the Learning.set, in order to validate the model by checking whether the standardized forecast errors follow a standard normal distribution.
...
for(ID in unique(Learning.set[,identifyer])){
  ...
  extracted <- extract.res(res = res, smot = NULL, relevant.names=c("log.d9.sal", "log.max.daily.range.sal","log0.mortality.rel.20"))
...
### Apply the multivariate DLM to the Test.set ----

## Now we wish to apply the DLM to the *Test.set* with the optimized parameters: mu0, C0, V, and W (V and W from the EM algorithm)

## Here we will also apply the Smoother to the DLM results (res). The retrospective smoothing analyses the data backwards, providing the best possible estimate of the true underlying levels of all variables given all available information prior to and after a given time step. Therefore, it gives the best estimates for "log.d9.sal", "log.max.daily.range.sal" and "log0.mortality.rel.20".
...
for(ID in unique(Test.set[,identifyer])){
  ...
  # Use the *extract.res* function to extract the output of the DLM (which is returned as lists) as a data frame, and call the resulting data frame "extracted".
  extracted <- extract.res(res = res, smot = smot, relevant.names=c("log.d9.sal","log.max.daily.range.sal","log0.mortality.rel.20"))
...
# Use a *for*-loop to iterate over each of the Test.set production cycles IDs:
## In this case, there are a lot of ID's so it may take some time. We suggest to decide on the ID's we want to see and run one by one. Run: ID=unique(Test.set[,identifyer])[353]
for(ID in unique(Test.set[,identifyer])){
  ...
  mt <- extracted.all.test[i, paste("mt_", name, sep="")] 
  ft <- extracted.all.test[i, paste("ft_", name, sep="")]
  Ct <- extracted.all.test[i, paste("Ct_", name, sep="")]      
  Qt <- extracted.all.test[i, paste("Qt_", name, sep="")]      
  mts <- extracted.all.test[i, paste("mts_", name, sep="")]     
  Cts <- extracted.all.test[i, paste("Cts_", name, sep="")]     
  
  min <- min(ft-1.96*sqrt(Qt), na.rm=T)
  max <- max(ft+1.96*sqrt(Qt), na.rm=T)
  
  #mt - filtered mean
  plot(ID.set[,name], type="l", xlab="month", ylab=name, 
  ...
  Obs_no_s <- (ID.set[,name] * SD.name) + Mean.name
  ...
  # - remove log transformation from most relevant names (log(x))
  ...
  # - remove log transformation from mortality (log(x+0.00005))
  ...
  #mt - filtered mean
  plot(Obs, type="l", xlab="month", ylab=name.no.log,
       main=paste("Filtered mean - nseq:", ID, sep=" "),
       ylim=c(min,max), lwd=2)                           # Observations
